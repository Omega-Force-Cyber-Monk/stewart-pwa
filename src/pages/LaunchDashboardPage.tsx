...
function Field({ label, value, onChange, type = "text", required = false, placeholder }: { label: string; value: string; onChange: (value: string) => void; type?: string; required?: boolean; placeholder?: string }) {
  return (
    <label className="block text-sm font-medium text-slate-700">
      ...
      <input required={required} type={type} value={value} placeholder={placeholder} onChange={(event) => onChange(event.target.value)} className="mt-2 h-11 w-full rounded-lg border border-slate-200 px-3 text-sm outline-none focus:border-dashboard-rider focus:ring-2 focus:ring-green-100" />
    </label>
  );
}
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl bg-white shadow-sm">
        ...
        <div className="border-t border-slate-100 p-5 sm:p-8">
          ...
          {step === 2 && <form onSubmit={submitBusiness} className="space-y-5 rounded-xl border border-slate-200 p-5"><div className="grid gap-5 md:grid-cols-2"><Field label="Business Name" required value={business.businessName} onChange={(value) => setBusiness((current) => ({ ...current, businessName: value }))} /><Field label="Phone Number" required value={business.phone} onChange={(value) => setBusiness((current) => ({ ...current, phone: value }))} /><Field label="Email Address" required type="email" value={business.email} onChange={(value) => setBusiness((current) => ({ ...current, email: value }))} /><label className="text-sm font-medium text-slate-700">Upload Business Logo (optional)<input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => setLogoFile(event.target.files?.[0] || null)} className="mt-2 block w-full rounded-lg border border-dashed border-slate-300 p-3 text-xs" /></label></div><label className="block text-sm font-medium text-slate-700">Business Information<span className="text-red-500">*</span><textarea required value={business.businessInfo} onChange={(event) => setBusiness((current) => ({ ...current, businessInfo: event.target.value }))} rows={4} className="mt-2 w-full rounded-lg border border-slate-200 p-3 text-sm outline-none focus:border-dashboard-rider" /></label>{!addonOwned && <div className="rounded-lg bg-slate-50 p-4"><label className="flex items-center gap-2 text-sm font-medium text-slate-700"><input type="checkbox" checked={acuity.connected} onChange={(event) => setAcuity((current) => ({ ...current, connected: event.target.checked }))} /> Connect Acuity Scheduling</label>{acuity.connected && <div className="mt-4"><Field label="Acuity Booking URL" required value={acuity.bookingUrl} onChange={(value) => setAcuity((current) => ({ ...current, bookingUrl: value }))} placeholder="https://acuityscheduling.com/..." /></div>}</div>}<div className="flex justify-between"><button type="button" onClick={() => setStep(1)} className="rounded-lg border border-slate-300 px-5 py-2.5 text-sm text-slate-600"><ChevronLeft className="mr-1 inline h-4 w-4" />Back</button><button className="rounded-lg bg-dashboard-rider px-5 py-2.5 text-sm font-semibold text-white" disabled={isSaving}>Save &amp; Continue</button></div></form>}

          {step === 3 && <form onSubmit={submitService} className="max-w-3xl space-y-5 rounded-xl border border-slate-200 p-5"><div className="relative"><label className="block text-sm font-medium text-slate-700">City or Metro Area<span className="text-red-500">*</span><input required value={serviceArea.cityArea} onChange={(event) => updateCity(event.target.value)} placeholder="Enter the city or metro area you serve" className="mt-2 h-11 w-full rounded-lg border border-slate-200 px-3 text-sm outline-none focus:border-dashboard-rider" /></label>{suggestions.length > 0 && <div className="absolute z-10 mt-1 w-full rounded-lg border border-slate-200 bg-white p-2 shadow-lg">{suggestions.map((suggestion) => <button type="button" key={suggestion.cityArea} onClick={() => chooseCity(suggestion)} className="block w-full rounded px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50"><span className="font-medium">{suggestion.cityArea}</span><span className="mt-0.5 block text-xs text-slate-500">{suggestion.airports.join(", ")}</span></button>)}</div>}</div><label className="block text-sm font-medium text-slate-700">Airports Served<span className="text-red-500">*</span><div className="mt-2 flex gap-2"><input value={airportSearch} onChange={(event) => setAirportSearch(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && airportSearch.trim()) { event.preventDefault(); setServiceArea((current) => ({ ...current, airports: Array.from(new Set([...current.airports, airportSearch.trim().toUpperCase()])) })); setAirportSearch(""); } }} placeholder="Type airport code and press Enter" className="h-11 flex-1 rounded-lg border border-slate-200 px-3 text-sm" /><Plane className="mt-3 h-4 w-4 text-slate-400" /></div></label><div className="flex flex-wrap gap-2">{serviceArea.airports.map((airport) => <span key={airport} className="inline-flex items-center gap-2 rounded-lg bg-slate-100 px-3 py-2 text-sm text-slate-700"><MapPin className="h-3.5 w-3.5" />{airport}<button type="button" onClick={() => setServiceArea((current) => ({ ...current, airports: current.airports.filter((item) => item !== airport) }))} aria-label={`Remove ${airport}`}>x</button></span>)}</div><div className="flex justify-between"><button type="button" onClick={() => setStep(2)} className="rounded-lg border border-slate-300 px-5 py-2.5 text-sm text-slate-600"><ChevronLeft className="mr-1 inline h-4 w-4" />Back</button><button className="rounded-lg bg-dashboard-rider px-5 py-2.5 text-sm font-semibold text-white" disabled={isSaving}>Save &amp; Continue</button></div></form>}

          {step === 4 && <form onSubmit={launch} className="space-y-5"><div className="grid gap-5 lg:grid-cols-2"><div className="rounded-xl border border-slate-200 p-5"><div className="flex items-center justify-between border-b border-slate-100 pb-4"><h4 className="font-semibold text-slate-900">Information Overview</h4><button type="button" onClick={() => setStep(1)} className="text-dashboard-rider" aria-label="Edit information"><Pencil className="h-4 w-4" /></button></div><dl className="mt-4 space-y-4 text-sm"><div><dt className="font-semibold text-slate-800">Personal Information</dt><dd className="mt-1 text-slate-500">{review?.buyer?.fullName || buyer.fullName}<br />{review?.buyer?.phone || buyer.phone}<br />{review?.buyer?.email || buyer.email}</dd></div><div><dt className="font-semibold text-slate-800">Business Information</dt><dd className="mt-1 text-slate-500">{review?.business?.name || business.businessName}<br />{review?.business?.phone || business.phone}<br />{review?.business?.email || business.email}</dd></div><div><dt className="font-semibold text-slate-800">Service Area Information</dt><dd className="mt-1 text-slate-500">{review?.serviceArea?.cityArea || serviceArea.cityArea}<br />{(review?.serviceArea?.airports || serviceArea.airports).join(", ")}</dd></div></dl></div><div className="rounded-xl border border-slate-200 p-5"><h4 className="border-b border-slate-100 pb-4 font-semibold text-slate-900">Generated Assets</h4><div className="mt-4 space-y-4 text-sm"><p><strong>Driver Website</strong><br />{websiteUrl ? <a href={websiteUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 break-all text-dashboard-rider">{websiteUrl}<ExternalLink className="h-3.5 w-3.5 shrink-0" /></a> : <span className="text-slate-500">Your website URL will appear after the business name is saved.</span>}</p><p><strong>Referral Card &amp; QR Code</strong><br /><span className="text-slate-500">Your QR code is generated automatically when you confirm launch.</span></p><p><strong>Selling Page</strong><br /><span className="text-slate-500">Your shareable landing page is ready to promote your services.</span></p><p><strong>Resources &amp; Guides</strong><br /><span className="text-slate-500">Marketing resources and launch materials are available to help you grow.</span></p>{!canLaunch && missingRequirements.length > 0 && <div className="rounded-lg bg-amber-50 p-3 text-sm text-amber-800">Missing: {missingRequirements.join(", ")}</div>}</div></div></div><div className="flex items-center justify-between"><button type="button" onClick={() => setStep(3)} className="rounded-lg border border-slate-300 px-5 py-2.5 text-sm text-slate-600"><ChevronLeft className="mr-1 inline h-4 w-4" />Back</button><button disabled={!canLaunch || isLaunching} className="rounded-lg bg-dashboard-rider px-6 py-3 text-sm font-semibold text-white disabled:opacity-50"><Rocket className="mr-2 inline h-4 w-4" />Confirm &amp; Launch</button></div></form>}
        </div>
      </section>
    </div>
  );
